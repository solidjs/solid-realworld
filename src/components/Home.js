...
export default () => {
  const [CommonStore, UserStore, ArticleStore] = useStore("common", "user", "article"),
    getTab = () => qsParse(window.location.search).tab || 'all',
    getTag = () => qsParse(window.location.search).tag || "",
    getPredicate = () => {
      switch (getTab()) {
        case 'feed': return { myFeed: true };
        case 'tag': return { tag: qsParse(window.location.search).tag };
        default: return {};
      }
    },
    handleTabChange = tab => {
    ...
    <div class="container page">
      <div class="row">
        ...
        <div class="col-md-9">
          <div class="feed-toggle">
            <ul class="nav nav-pills outline-active">
              <Show when={(UserStore.state.currentUser)}>
                <li class="nav-item">
                  <NavLink class="nav-link" route="?tab=feed">Your Feed</NavLink>
                </li>
              </Show>
              <li class="nav-item">
                <NavLink class="nav-link" route="?tab=all">Global Feed</NavLink>
              </li>
              <Show when={(props.tag)}>
          ...
          <ArticleList
            articles={(ArticleStore.state.articles)}
            loading={(ArticleStore.state.isLoading)}
            totalPagesCount={(ArticleStore.state.totalPagesCount)}
            currentPage={(ArticleStore.state.page)}
        ...
        <div class="col-md-3">
          <div class="sidebar">
            ...
            <Show when={(CommonStore.state.tags)}>
              <div class="tag-list">
                <For each={(CommonStore.state.tags)}>{ tag =>
                  <a href={`#/?tab=tag&tag=${tag}`} class="tag-pill tag-default">{tag}</a>
                }</For>
              </div>
            </Show>
          </div>
